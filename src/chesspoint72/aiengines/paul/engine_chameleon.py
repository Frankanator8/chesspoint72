"""engine_chameleon.py — Paul's Chameleon hybrid engine.
...
"personalities" for middlegame and endgame play:
  ...
  1. **Evaluator** — PhaseSwitchingEvaluator routes to nnue_tank when
     non-king piece count > 12, otherwise nnue_finisher. The swap is
     evaluated per node so the transition is smooth.

  2. **Move ordering** — PhaseAwareOrdering consults the same threshold and
  ...
  3. **Pruning policy** — two ForwardPruningPolicy instances, both attached
...
Compared to the Sentry — which also pairs tank+finisher — the Chameleon is
...
class ChameleonSearch(NegamaxSearch):
    ...
    def __init__(
        ...
        # Pre-attach BOTH policies — the parent only attached the bootstrap.

"""engine_cannon.py — Paul's Glass Cannon hybrid engine.
...
Distinguishing features (each unique to this engine in Paul's suite):
  ...
  - **CannonSearch**: two architectural pieces unique to this engine —
      1. **Narrow aspiration windows (Δ=30)**: tighter than Chaos's Δ=50.
  ...
  - **cannon_pruning_config**: ultra-aggressive — razoring disabled (its
...
Default depth 6 / time 3s — tuned for short-clock tactical knockouts.
...
_INF = 10_000_000
_ASPIRATION_DELTA = 30  # tighter than Chaos's 50 — the tactician justifies it

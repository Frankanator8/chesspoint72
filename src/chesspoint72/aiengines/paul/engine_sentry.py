"""engine_sentry.py — Paul's Iron Sentry hybrid engine.
...
Distinguishing features (each unique to this engine in Paul's suite):
  ...
  - **DualPhaseEvaluator**: routes evaluate_position() to nnue_tank when
    ...
    Chameleon, this is the *only* phase-aware element here — the search
  ...
  - **SentrySearch**: enforces a **minimum-depth guarantee** — the first 4
